...
import { friendlyErrorMessage, isPromptTooLongError, isThinkingSignatureError, mapSDKErrorToTypedError } from './adapters/pi-agent-adapter'
...
export class AgentOrchestrator {
   ...
   * 构建 Pi 工具运行环境。
   *
   * 模型凭据由 Pi provider registry 显式接收，不再写入全局 process.env，
   * 也不向 Bash/MCP 子进程注入 Claude CLI 专用变量。
   */
  private buildPiRuntimeEnv(proxyUrl: string | undefined): AgentRuntimeEnv {
    const cleanEnv: Record<string, string | undefined> = {}
    for (const [key, value] of Object.entries(process.env)) {
      if (!key.startsWith('ANTHROPIC_') && !key.startsWith('CLAUDE_CODE_') && key !== 'CLAUDE_BASH_NO_LOGIN') {
        cleanEnv[key] = value
      }
    }
    ...
    return {
      ...runtimeEnv,
      env: mergeRuntimeEnv(cleanEnv, runtimeEnv.env),
    }
  }
  ...
  async sendMessage(input: AgentSendInput, callbacks: SessionCallbacks): Promise<void> {
    ...
    // 3. 构建 Pi 工具运行环境。模型凭据通过 query options 传递，不污染全局环境。
    const proxyUrl = await getEffectiveProxyUrl()
    const runtimeEnv = this.buildPiRuntimeEnv(proxyUrl)
    ...
    try {
      ...
      // Pi 由 Proma 主进程连接用户 MCP server，并转换为 customTools。
      if (Object.keys(mcpServers).length > 0) {
        try {
