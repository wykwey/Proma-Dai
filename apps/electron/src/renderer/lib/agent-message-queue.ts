...
export type QueuedMessageReferenceType = 'file' | 'skill' | 'mcp' | 'session'
...
const DISPLAY_REFERENCE_PATTERN = /@file:(?<file>\S+)|\/skill:(?<skill>\S+)|#mcp:(?<mcp>\S+)|&session:(?<session>[A-Za-z0-9-]+)(?:(?:~|::)(?<sessionLabel>\S+))?/g
...
/**
 * 将排队消息中的文件、Skill、MCP 和会话协议转换为展示片段。
