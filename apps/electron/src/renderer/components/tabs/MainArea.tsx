...
export function MainArea(): React.ReactElement {
  ...
  // 左侧容器宽度：预览面板打开时固定占 splitRatio；其他情况（含 closing 动画期间）
  // 直接 1 1 auto 占满——closing 时右侧 absolute 脱离 flex 流，所以左侧自然占 100%。
  const showRightPanel = showPreviewPane
      ...
      >
        <div className="flex flex-1 min-h-0 relative overflow-hidden" data-split-container>
          ...
          >
            {activeView === 'planning' ? (
              automationFormOpen ? (
                // 自动化设置页：与定时任务同层级替换中间区，不经过 TabBar。
                <AutomationFormView />
              ) : (
                <AutomationsListView />
              )
          ...
          {showRightPanel && (
            <div
              className={cn(closing ? 'animate-preview-slide-out' : 'flex flex-1 min-w-0')}
              style={closing ? closingOverlayStyle : undefined}
              onAnimationEnd={(e) => {
