...
Proma is a local-first AI desktop app that brings multi-model Chat, general-purpose Agent workflows, workspaces, Skills, MCP, and memory into one open-source client.
...
- **Memory**: Chat and Agent can share memory.
...
- **Desktop experience**: auto-update, proxy settings, file preview, global shortcuts, quick task window, and light / dark / system themes.
...
The open-source version can be used independently with self-configured AI provider channels and API keys.
...
7. Configure memory or web search from their corresponding settings tabs if needed.
...
- Tasks that benefit from permissions, plan mode, or background execution.
...
- `document-parser.ts`, `workspace-watcher.ts`: document parsing, and file watching.
