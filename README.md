...
Proma 是一个本地优先的 AI 桌面应用，把多模型 Chat、通用 Agent、工作区、Skills、MCP 和记忆能力放在同一个开源客户端里。
...
- **记忆**：Chat 和 Agent 可共享记忆能力。
- **本地优先**：会话、工作区、附件、配置、Skills 等默认存储在 `~/.proma/`，使用 JSON / JSONL 文件组织，不依赖本地数据库。
- **桌面体验**：自动更新、代理设置、文件预览、全局快捷键、快速任务窗口、亮色 / 暗色 / 跟随系统主题。
...
开源版可独立使用，支持自行配置 AI 供应商渠道与 API Key。
...
- `document-parser.ts`、`workspace-watcher.ts`：文档解析和文件监听。
